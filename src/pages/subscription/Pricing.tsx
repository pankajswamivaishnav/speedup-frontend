...
import { Button, CircularProgress, Grid, List, ListItem, ListItemIcon, ListItemText, Stack, Switch, Typography } from '@mui/material';
...
const plans = [
  {
    ...
    price: 99,
    planId: '6958b6cd0d52dd2e1706d384',
    permission: [0, 1]
  },
  {
    active: true,
    ...
    price: 499,
    planId: '6958b6cd0d52dd2e1706d385',
    permission: [0, 1, 2, 3]
  },
  {
    active: false,
    ...
    price: 999,
    planId: '6958b6cd0d52dd2e1706d386',
...
const Pricing = () => {
  ...
  const verifyPayment = async (paymentResponse: any, planId: string) => {
    try {
      await planServiceInstance.verifyPayment({
  ...
  const handleOrderNow = async (planId: string) => {
    try {
      ...
      const response = await planServiceInstance.createOrder(planId);
      setLoadingPlanId('');
      const options = {
      ...
      console.log('errors in handle order now', error);
      setLoadingPlanId('');
    }
  };
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <Button
                    ...
                    onClick={() => handleOrderNow(plan.planId)}
                  >
                    {loadingPlanId === plan.planId ? <CircularProgress size={22} color="inherit" /> : 'Order Now'}
                  </Button>
                </Grid>
                <Grid item xs={12}>
                  <List
